...
import { Console, Duration, Effect, Scope, pipe } from "effect";
...
export class CedroDumper {
  ...
  public startNewDump(prefix = "cedro-dump"): string {
    ...
    // Create write stream
    this.fileStream = fs.createWriteStream(this.currentDumpFile, {
      flags: "a",
    });
    this.messageCount = 0;
    ...
    this.fileStream.write(
      `# Cedro Data Dump - Started at ${new Date().toISOString()}\n`
    );
  ...
  public static filterDump(
    ...
    // Write header
    outputStream.write(
      `# Filtered Cedro Data - Generated at ${new Date().toISOString()}\n`
    );
  ...
  public static analyzeDump(dumpFile: string): Record<string, unknown> {
    ...
    for (const line of lines) {
      if (line?.startsWith("# Cedro Data Dump - Started at")) {
        ...
        stats.totalMessages = Number.parseInt(
          line.replace("# Total messages: ", ""),
          10
        );
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      stats.messageTypes[messageType] =
        (stats.messageTypes[messageType] || 0) + 1;
